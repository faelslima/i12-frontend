...
import {EncontreiroModel} from '../../../../../shared/models/encontreiro.model';
import {EncontreiroService} from '../../../../../shared/service/encontreiro.service';
import {EncontroEquipeEnum} from '../../../../../shared/enumetrations/encontro-equipe.enum';
...
@Component({
    ...
    templateUrl: './edit-encontreiro-modal.component.html',
    styleUrls: ['./edit-encontreiro-modal.component.scss'],
    providers: [
        {provide: NgbDateAdapter, useClass: CustomAdapter},
        {provide: NgbDateParserFormatter, useClass: CustomDateParserFormatter}
    ]
})
export class EditEncontreiroModalComponent implements OnInit, OnDestroy {
    @Input() id: number | string;
    @Input() encontro: EncontroModel;
    isLoading$;
    entity: EncontreiroModel;
    formGroup: FormGroup;
    formGroupEncontreiros: FormGroup;
    igrejas: Array<IgrejaModel>;
    descendencias: Array<DescendenciaModel>;
    encontreiros: PessoaModel[];
    pessoas: PessoaModel[];
    equipes: Array<EncontroEquipeEnum>;
    ...
    private EMPTY_ENTITY: EncontreiroModel;
    private subscriptions: Subscription[] = [];

    constructor(
        private igrejaService: IgrejaService,
        private entityService: EncontreiroService,
    ...
    ngOnInit(): void {
        this.EMPTY_ENTITY = new EncontreiroModel();
    ...
    private loadEntity() {
        if (!this.id) {
            ...
            ).subscribe((object: EncontreiroModel) => {
                this.entity = object;
    ...
    private loadForm() {
        ...
        this.formGroup = this.fb.group({
            ...
            equipe: [this.entity?.equipe, Validators.compose([Validators.required])],
    ...
    create() {
        ...
        ).subscribe((res: EncontreiroModel) => {
            if (res) {
                Swal.fire({
    ...
    private prepareNovaVida() {
        ...
        this.entity.equipe = formData.equipe;
    ...
    private initEnums() {
        ...
        this.equipes = [];
        this.equipes.push(EncontroEquipeEnum.APOIO);
        this.equipes.push(EncontroEquipeEnum.INTERCESSAO);
        this.equipes.push(EncontroEquipeEnum.COZINHA);
        this.equipes.push(EncontroEquipeEnum.SOM);
        this.equipes.push(EncontroEquipeEnum.STAND_BY);
        this.equipes.push(EncontroEquipeEnum.CORREIOS);
        this.equipes.push(EncontroEquipeEnum.OUTRO);
    }
    ...
    private loadFilterEncontreiros() {
        this.formGroupEncontreiros = this.fb.group({
            pessoa: ['']
        });

        this.formGroupEncontreiros.controls.pessoa.valueChanges.pipe(
            debounceTime(500),
            tap(() => {
                this.pessoas = [];
                this.isLoadingPessoa = true;
            ...
            if (values) {
                this.encontreiros = values;
            }
        });
    }

    onChangeFilterEncontreiro(value) {
        if (value) {
            this.entity = value;
        }
    }
